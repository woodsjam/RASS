...
Well, that means that location is correlated with ethnicity family structure and all that other stuff. Looks like we need another model.  We have to describe locational choice based on some of those observable factors. 
